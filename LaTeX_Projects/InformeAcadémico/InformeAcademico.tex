...
La verificación responde a la pregunta: ¿el modelo fue construido correctamente? Su protocolo se instrumentalizó como una lista de chequeo PASS/FAIL aplicada antes de ejecutar los experimentos. La Tabla~\ref{tab:verificacion} resume las pruebas ejecutadas, la condición esperada y el resultado obtenido.

\begin{table}[H]
\centering
\caption{Protocolo de verificación: pruebas y resultados}
...
\color{white}\textbf{Condición esperada} &
\color{white}\textbf{Estado} \\
\midrule
\rowcolor{tabrow} Secuencia de etapas por lote & Los lotes recorren Pesado→Amasado→Reposo→Formado→Ferment.→Horneado→Enfriado en ese orden. & \textcolor{verdeok}{\textbf{PASS}} \\
Captura/liberación del panadero & El panadero se asigna en Pesado, Amasado y Formado; se libera al inicio del Reposo pasivo. & \textcolor{verdeok}{\textbf{PASS}} \\
\rowcolor{tabrow} Rechazo de familias mixtas & El horno rechaza cargas con lotes de más de una familia en la misma corrida. & \textcolor{verdeok}{\textbf{PASS}} \\
Límite de carros y bandejas & La carga de cada corrida no excede 6~carros ni 108~bandejas totales. & \textcolor{verdeok}{\textbf{PASS}} \\
\rowcolor{tabrow} Descansos escalonados & Los descansos de panaderos y manipuladores nunca son simultáneos para la misma función. & \textcolor{verdeok}{\textbf{PASS}} \\
No hay colas infinitas ni bloqueos & Inspección visual en warm-up: ninguna cola crece sin límite y no hay entidades atrapadas. & \textcolor{verdeok}{\textbf{PASS}} \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{Cuadro comparativo de escenarios (dashboard)}

\begin{table}[H]
\centering
\caption{Dashboard: KPIs clave por escenario}
\label{tab:dashboard-acad}
\renewcommand{\arraystretch}{1.35}
...
Caso~A & \cellcolor{rojoriesgo!20}84,7\% & 7 & 1.244 & 1.251 & 34/2 & \textcolor{naranja}{Mejor realista} \\
\rowcolor{tabrow} Caso~B & \cellcolor{rojoriesgo!20}79,1\% & 8 & 1.693 & 1.697 & 29/4 & \textcolor{rojoriesgo}{Peor que A} \\
Caso~C & \cellcolor{rojoriesgo!20}79,3\% & 8 & 1.680 & 1.681 & 31/4 & \textcolor{rojoriesgo}{$\approx$ Caso~B} \\
\rowcolor{tabrow} Caso~D & \cellcolor{verdeok!25}98,6\% & 4 & 112 & 110 & 35/0 & \textcolor{grisoscuro}{Techo teórico} \\
\bottomrule
\end{tabularx}
\end{table}

\noindent\textit{Nota:} «Prod.~$<$95\%» = número de tipos de pan cuyo NDS individual no alcanza el umbral. El criterio del estudio es simultáneo: todos deben superar el 95\%.
