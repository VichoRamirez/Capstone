...
La lógica del modelo se implementa en ocho procesos con triggers explícitos. La siguiente tabla describe qué hace cada uno y cuándo se activa:

\begin{table}[H]
\centering
\caption{Procesos del modelo: función y momento de activación}
\label{tab:procesos}
\renewcommand{\arraystretch}{1.3}
...
\color{white}\textbf{Activado por} \\
\midrule
\rowcolor{tabrow}\texttt{P\_InitModel}              & Inicializa el vector de stock \texttt{StockSala[i]} con los valores de \texttt{Tbl\_StockPolitica}, carga los parámetros de escenario activo y arranca los timers de producción push. & Inicio de corrida (hora~0) \\
\texttt{P\_GenerarDemandaCliente}   & Sortea el número de tipos de pan a comprar (1, 2 ó 3) según las probabilidades del supuesto de mezcla de compra. Crea las condiciones para la cadena de elección y consumo. & Llegada de entidad cliente desde \texttt{Source\_Clientes} \\
\rowcolor{tabrow}\texttt{P\_ElegirProductos}        & Selecciona sin reemplazo los índices de producto a comprar en esa visita, ponderando por las probabilidades horarias de \texttt{Tbl\_ProbEleccionHoraLong}. & Salida de \texttt{P\_GenerarDemandaCliente} \\
\texttt{P\_ConsumirInventario}      & Descuenta \texttt{StockSala[i]} en la cantidad comprada. Si hay stock suficiente, acumula \texttt{demanda\_satisfecha\_kg}; si el stock es $\leq 0$, acumula \texttt{demanda\_perdida\_kg} y registra un stockout. Si hay quiebre, dispara \texttt{P\_EvaluarHorno} inmediatamente (modo pull). & Salida de \texttt{P\_ElegirProductos} \\
\rowcolor{tabrow}\texttt{P\_EvaluarHorno}           & Evalúa si algún producto requiere un lote de producción: calcula la cobertura proyectada de stock para cada producto y genera un nuevo lote si está por debajo del umbral. Ordena los productos por la política activa (\texttt{MenorStockRelativo}: menor \texttt{StockSala[i] / DemandaMedia[i]}). & Timer periódico (push) o quiebre detectado (pull) \\
\texttt{P\_FormarCargaHorno}        & Agrupa lotes pendientes de la misma familia hasta alcanzar la capacidad máxima de carros (6) o el tiempo de espera máximo (15~min). Genera la corrida de horno cuando se cumple cualquiera de las dos condiciones: carga mínima de 600~kg o espera máxima vencida. & Llegada de lote al área de espera del horno \\
\rowcolor{tabrow}\texttt{P\_AplicarSetupHorno}      & Compara la familia de la corrida actual con la familia de la corrida anterior. Si son distintas, aplica 5~min de setup; si son iguales, el setup es 0~min. Acumula \texttt{setup\_horno\_count} y \texttt{tiempo\_setup\_total\_min}. & Antes de cada corrida de horno \\
\texttt{P\_ReponerSala}             & Incrementa \texttt{StockSala[i]} en los kilogramos del lote recién completado. Se ejecuta cuando el lote llega al nodo final tras el enfriado. El incremento es inmediato (sin demora de traslado adicional). & Llegada del lote al nodo \texttt{Sink\_Sala} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{Caso~D muestra el techo}: con lotes óptimos se alcanzan 98,6\%, pero 4 productos aún no superan el 95\% (Baguette 93,9\%, Dobladita 92,2\%, Bocado de Dama 89,2\%, Amasado 94,5\%), lo que indica que el sistema necesita más tiempo de operación (inicio más temprano) para los productos de menor demanda y ciclo largo.
\end{itemize}
...
\color{white}\textbf{Impacto esperado} &
\color{white}\textbf{Prioridad} \\
\midrule
\rowcolor{tabrow}
1 &
Contratar 1~panadero adicional (total: 3) &
Nivel global sube de 68,3\% a 84,7\% ($+16,4$~pp). Marraqueta Integral y Hallulla Integral alcanzan 100\%. &
\textcolor{rojoriesgo}{\textbf{Urgente}} \\

2 &
Adelantar inicio de producción a 04:00--05:00 &
Permite 1--2 ciclos adicionales antes del primer pico de demanda (12:00--14:00). Proyecta resolver los 5 productos actualmente limitados por ciclos. &
\textcolor{rojoriesgo}{\textbf{Urgente}} \\

\rowcolor{tabrow}
3 &
Agregar 1~manipulador (total: 2) &
Destraba la carga/descarga del horno y acelera la reposición a sala. Libera capacidad efectiva del panadero. &
\textcolor{naranja}{\textbf{Alta}} \\

4 &
Revisar política de secuenciamiento de lotes &
Priorizar productos con menor cobertura proyectada. La política híbrida actual es correcta pero puede refinarse. &
\textcolor{naranja}{\textbf{Alta}} \\

\rowcolor{tabrow}
5 &
Evaluar 2.° horno SOLO tras implementar R1--R3 &
El Caso~B demuestra que agregar el horno \emph{antes} de escalar el personal reduce el nivel de servicio de 84,7\% a 79,1\%. El 2.°~horno solo aporta valor si los panaderos y manipuladores ya están dimensionados correctamente. &
...
\begin{itemize}
  \item Reducción de quiebres de stock en más del 95\% (de 2.568 a menos de 200 eventos/día proyectados).
  \item Recuperación de ventas directas equivalente a la demanda actualmente perdida.
  \item Mejora en la experiencia de compra: disponibilidad consistente de todos los tipos de pan durante toda la jornada.
  \item Bajo riesgo de sobreproducción: el modelo indica que el sistema trabaja cerca del límite de capacidad, no genera excedentes excesivos.
\end{itemize}
